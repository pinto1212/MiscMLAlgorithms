...
class MDP:
    ...
    def __init__(self, environment, convergence_threshold=0.0001, gamma=0.85, max_iterations=1e8, grid=False, seed = None):
        self.env_name = environment
        self.env = gym.make(environment)
        #self.env = GamblersEnv()
    ...
    def policy_iteration(self, iterations_to_save=[0], visualize=False):
        ...
        if visualize and iteration == iterations_to_save[saving_index % len(iterations_to_save)]:
            ...
            utils.set_plot_title_labels(title='Policy at iteration = {}'.format(1))
            utils.save_figure('{}_PI_policy_iter{}'.format(self.env_name, 1))
            saving_index += 1
        ...
        while iteration <= self.max_iterations and improve_policy:
            ...
            if visualize and iteration == iterations_to_save[saving_index % len(iterations_to_save)]:
                ...
                utils.set_plot_title_labels(title='Policy at iteration = {}'.format(iteration+1))
                utils.save_figure('{}_PI_policy_iter{}'.format(self.env_name, iteration+1))
                saving_index += 1
    ...
    def Q_learning(self, num_episodes=1000, learning_rate_decay=0.99, epsilon_decay=1, visualize=False, epsilon_ini=1.0):
        ...
        episode = 0
        epsilon, learning_rate = epsilon_ini, 1.0
        ...
        # f=open('test.out','ab')
        # np.savetxt(f, average_scores, delimiter=',')
        ...
        if visualize:
            ...
            utils.set_plot_title_labels(title='Q Learning - Accumulated reward',
                                        x_label='Iteration', y_label='Accumulated reward')
    ...
    def value_iteration(self, iterations_to_save=[1], visualize=False):
        ...
        if visualize:
            utils.set_plot_title_labels(title='Value iteration - State Value function by iteration',
                                        x_label='State', y_label='V', legend=True)
            utils.save_figure('{}_VI_iterations'.format(self.env_name))

            plt.figure(figsize=(10, 10))
            utils.plot_value_convergence(deltas, iteration)
            utils.set_plot_title_labels(title='Value Iteration - Value vs Iteration',
                                        x_label='Iteration', y_label='diff Value')
        ...
        if visualize:
            ...
            utils.set_plot_title_labels(title='Value Iteration - Optimal State Utility')
            ...
            utils.set_plot_title_labels(title='Value Iteration - Optimal Policy')
