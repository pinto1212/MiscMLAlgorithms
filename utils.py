...
def plot_clusters(ax, component1, component2, df, name, dataset):
    ...
    y_colors = sns.color_palette('hls', len(np.unique(df['label'])))
    c_colors = sns.color_palette('hls', len(np.unique(df['cluster'])))
    sns.scatterplot(x=component1, y=component2, hue='label', palette=y_colors, data=df, legend='full', alpha=0.3, ax=ax[0])
    sns.scatterplot(x=component1, y=component2, hue='cluster', palette=c_colors, data=df, legend='full', alpha=0.3, ax=ax[1])

    # Set titles
    ax[0].set_title('{} - True labels - {}'.format(dataset, component1[:-1].upper()))
    ax[1].set_title('{} - {} Clusters - {}'.format(dataset, name.upper(), component1[:-1].upper()))
...
def plot_components(component1, component2, df, name):
    ...
    plt.figure()
    colors = sns.color_palette('hls', len(np.unique(df['label'])))
    sns.scatterplot(x=component1, y=component2, hue='label', palette=colors, data=df, legend='full', alpha=0.3)

    # Annotate standard deviation arrows for the two components
    plt.annotate('', xy=(np.std(df[component1]), 0), xytext=(0, 0), arrowprops=dict(arrowstyle='->', color='red', lw=3))
    plt.annotate('', xy=(0, np.std(df[component2])), xytext=(0, 0), arrowprops=dict(arrowstyle='->', color='blue', lw=3))

    # Set title and axes limits
    plt.title('First 2 {} components and true labels'.format(name.upper()))
    xlim = 1.1 * np.max(np.abs(df[component1]))
    ylim = 1.1 * np.max(np.abs(df[component2]))
    plt.xlim(-xlim, xlim)
    plt.ylim(-ylim, ylim)
...
def plot_multiple_random_runs(x_axis, y_axis, label):
    ...
    plot = plt.plot(x_axis, y_mean, '-o', markersize=1, label=label, color = 'black')
    plt.fill_between(x_axis, y_mean - y_std, y_mean + y_std, alpha=0.1, color='gray')


def plot_ic_bars(ic, ic_label, cv_types, k_range, ax, dataset):
    ...
    for i, cv_type in enumerate(cv_types):
        ...
        ax.plot(x, ic[i * len(k_range):(i + 1) * len(k_range)], label=cv_type)
    ...
    set_axis_title_labels(ax=ax, title='{} - EM - {} versus K'.format(dataset, ic_label),
                          x_label='K', y_label='{}'.format(ic_label))
