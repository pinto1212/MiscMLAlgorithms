...
class Clustering(ABC):
    ...
    def visualize_clusters(self, x, y, dataset):
        ...
        utils.plot_clusters(ax2, 'tsne1', 'tsne2', df, self.name, dataset)
...
class KMeansClustering(Clustering):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        utils.set_axis_title_labels(ax[0], title='{}-K-MEANS - Inertia versus K'.format(dataset),
                                    x_label='K', y_label='Inertia')

        # Plot silhouette for the different k values
        self.visualize_silhouette(x, ax, dataset)

        # Save figure
        utils.save_figure_tight('{}_{}_model_complexity'.format(dataset, self.name))

    def visualize_silhouette(self, x, ax, dataset):
        ...
        for k in range(2, self.max_n_clusters + 1):
            ...
            ax[k-1].axvline(x=silhouette_avg, color="blue", linestyle="--")
            ...
            utils.set_axis_title_labels(ax[k-1], title='{} - K = {}, Silhouette'.format(dataset, k),
                                        x_label='Silhouette', y_label='Silhouette per Cluster')
...
class MixtureOfGaussians(Clustering):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        cv_types = ['diag', 'spherical', 'full', 'tied']  # covariance types to plot over
        ...
        utils.plot_ic_bars(np.array(aic), 'AIC', cv_types, k_range, ax1, dataset)
        utils.plot_ic_bars(np.array(bic), 'BIC', cv_types, k_range, ax2, dataset)
