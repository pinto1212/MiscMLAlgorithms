...
class IndependentComponents(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        ax.plot(k_range, average_kurtosis,  '-o', markersize=1, label='Kurtosis')

        # Set title and labels
        utils.set_axis_title_labels(ax, title='{} - ICA - Average kurtosis versus K'.format(dataset),
                                    x_label='K', y_label='Average Kurtosis')
...
class KernelPrincipalComponents(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        for i, kernel in enumerate(kernels):
            ...
            ax[2*i].bar(k_range, np.cumsum(explained_variance_ratio), color='blue', label=kernel)

            # Set title, labels and legend
            ax[2*i].legend(loc='best')
            utils.set_axis_title_labels(ax[2*i], title='{} - KPCA - Cumulative variance versus K'.format(dataset),
                                        x_label='K', y_label='Cumulative Variance (%)')

            # Plot histogram of the explained variance ratio, i.e. the eignevalues distribution
            ax[2*i+1].bar(k_range, explained_variance_ratio, color='red', label=kernel)

            # Set title, labels and legend
            ax[2*i+1].legend(loc='best')
            utils.set_axis_title_labels(ax[2*i+1], title='KPCA - Eigenvalues variance distribution',
                                        x_label='K', y_label='Variance (%)')
...
class PrincipalComponents(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        ax.bar(k_range, np.cumsum(pca.explained_variance_ratio_), color='blue')
        utils.set_axis_title_labels(ax, title='{} - PCA - Cumulative explained variance'.format(dataset),
                                    x_label='K', y_label='Cumulative Variance (%)')

        # Plot histogram of the explained variance ratio, i.e. the eignevalues distribution, with titles and labels
        # ax.plot(k_range, pca.explained_variance_ratio_, color='cyan')
        # utils.set_axis_title_labels(ax, title=' {} - PCA - Eigenvalue variance distribution'.format(dataset),
        #                             x_label='k', y_label='Variance (%)')
...
class RandomProjections(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        utils.set_plot_title_labels(title='{} - RP - Reconstruction Error versus K'.format(dataset),
                                    x_label='K', y_label='Reconstruction Error (MSE)')
