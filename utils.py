...
def plot_heatmap_policy(policy, V, rows, columns):
    ...
    sns.heatmap(V.reshape(rows, columns), annot=policy_labels, fmt='', linewidths=.5, cmap="YlGnBu")


def plot_heatmap_value_function(V, rows, columns):
    sns.heatmap(V.reshape(rows, columns), annot=True, fmt='.3f', linewidths=.5, cmap="YlGnBu")
...
def plot_value_convergence(convergence, iteration):
    """Plot helper.
        ...
        Returns:
          None.
        """
    plt.plot(np.arange(1, iteration + 1), convergence, color='red')
...
def plot_value_function(V, iteration, show_label=False):
    """Plot helper.
        ...
        """
    if show_label:
        plt.plot(np.arange(1, len(V) + 1), V, label='{}{} Iteration'.format(iteration, 'st' if iteration == 1 else "th"))
    else:
